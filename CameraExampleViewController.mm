...
- (void)setPredictionValues:(std::vector<std::pair<float, std::string>>)newValues {
  ...
  for  (const auto& item : newValues) {
    ...
    std::string symbol = item.second;
    //std::string latexCode;
    NSString *latexCode = dictionary[symbol];
    std::string label ([latexCode UTF8String]);
