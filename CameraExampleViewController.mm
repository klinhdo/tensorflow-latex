...
static NSString* latex_file_name = @"latex";
static NSString* latex_file_type = @"txt";
...
- (void)viewDidLoad {
    ...
    LoadLabels(latex_file_name, latex_file_type, &latexs);
    
    //create a look up dictionary
    
    int dictionaryCount = (int) latexs.size();
    for (int i = 0; i < dictionaryCount; i++){
        string symbol = labels[i];
        string latexCode = latexs[i];
        dictionary.emplace(symbol, latexCode);
    }
...
- (void)setPredictionValues:(std::vector<std::pair<float, std::string>>)newValues {
    ...
    for  (const auto& item : newValues) {
        //here add in code that looks up a dictionary
        std::string label = dictionary.at(item.second);
